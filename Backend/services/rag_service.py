...
def retrieve_documents(query: str, user_id: str, k: int = 4) -> List[str]:
    ...
    """
    print(f"[DEBUG][RAG] Query: {query}, User ID: {user_id}, k: {k}") # 디버그 출력
    # 사용자 ID 필터링 없이 관련 문서를 검색합니다.
    docs = vector_store.similarity_search(query, k=k)
    print(f"[DEBUG][RAG] Raw retrieved documents ({len(docs)}): {docs}") # 디버그 출력
    ...
    for doc in docs:
        ...
        print(f"[DEBUG][RAG] Comparing user_id: {user_id} (type: {type(user_id)}) with doc_user_id: {doc_user_id} (type: {type(doc_user_id)})") # 디버그 출력
        if str(doc_user_id) == str(user_id):
            filtered_docs.append(doc)
    print(f"[DEBUG][RAG] Filtered documents by user_id ({len(filtered_docs)}): {filtered_docs}") # 디버그 출력
    
    # k개로 제한하여 반환합니다.
    final_docs = [doc.page_content for doc in filtered_docs[:k]]
    print(f"[DEBUG][RAG] Final documents for context ({len(final_docs)}): {final_docs}") # 디버그 출력
    return final_docs
